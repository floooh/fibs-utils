...
function getSlang(ctx: fibs.Context, slang: string | undefined): string {
    if (slang !== undefined) {
        ...
        switch (ctx.config.platform) {
            case 'android': return 'gles300es:glsl100';
            case 'ios': return 'gles300es:metal_ios:metal_sim';
            case 'emscripten': return 'gles300es:glsl100:wgpu';
            case 'linux': return 'glsl330:gles300es';
            case 'macos': return 'metal_macos';
            case 'windows': return 'hlsl4:glsl330';
            default: return 'glsl330';
        }
    }
}
            ...
            func: async (inputs: string[], outputs: string[], args: SokolShdcArgs) => {
                if (fibs.util.dirty(inputs, outputs)) {
                    const shdcArgs = [
                        ...
                        '--output', outputs[0],
                        '--genver', `${VERSION}`,
                        '--slang', getSlang(ctx, args.slang),
                        '--errfmt', getErrFmt(ctx, args.errfmt),
                        '--format', args.fmt!,
                        '--bytecode',
                    ];
